...
const handleLogin = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}${ENDPOINTS.USERS}?email=eq.${encodeURIComponent(email.trim())}`,
      { headers: getHeaders() }
    );
    ...
    const supabaseUser = encontrados[0];

    // 2. ¿Existe y coincide la contraseña?
    ...
    try {
      const bloqueoResponse = await fetch(
        `${API_URL}${ENDPOINTS.BLOQUEOS}?usuario_id=eq.${supabaseUser.id}&activo=eq.true`,
        { headers: getHeaders() }
      );
      ...
      // si falla la consulta de bloqueo, dejamos pasar en vez de trabar el login
    }

    // 4. ¿Está activo?
    if (supabaseUser.activo === false) {
      setError("❌ Esta cuenta está desactivada");
      return;
    }

    // 5. Armar el usuario de sesión
    const user = {
      id: supabaseUser.id,
      nombre: supabaseUser.nombre,
      email: supabaseUser.email,
      telefono: supabaseUser.telefono || "",
      accountType: supabaseUser.rol || "usuario",
      createdAt: supabaseUser.fecha_registro || new Date().toISOString(),
    };
